...
#rowSums(x2);colSums(x2)
apply(x2, 1, max) # 1일 경우 행연산
apply(x2, 1, min)
apply(x2, 1, mean)

apply(x2, 2, max) # 2일 경우 열 연산
apply(x2, 2, min)
apply(x2, 2, mean)
...
week.korabbname <- c("일", "월", "화",
                     "수", "목", "금", "토")
day2 <- factor(data1, 
               levels=week.korabbname) # 데이터에는 없어도 필요할 경우
day2
summary(day2)
levels(day2)
...
# 내장 데이터셋
data()
iris; head(iris);tail(iris) # tail(iris, 10) 보일 개수 지정 가능
...
fruit[, c(3,4)] # drop을 안줘도 형태를 유지해줌
fruit[3,2]
fruit[3,1]
...
#csv파일열기
score <- read.csv("data/score.csv") # 상대패스 현재 디렉토리 기준
# score <- read.csv("c://kdc/Rexam/data/score.csv") # 절대패스
score
str(score)
score$sum <- 
...
emp <- read.csv(file.choose()) # 유저가 선택해서 가져올 수 있다
emp
str(emp)
